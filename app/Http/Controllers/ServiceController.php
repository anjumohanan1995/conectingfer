...
class ServiceController extends Controller
{
    ...
    if ($request->hasfile('image')) {
        ...
        $images->move(public_path('/Services'), $img);
    ...
    return redirect()->route('service_list.index')->with('status', 'Created successfully');
}

    public function show(Service $service_list)
    {
    ...
    if ($request->hasfile('image')) {
        ...
        return redirect()->route('service_list.index')->with('status','Updated successfully.');
    }

    public function destroy($id)
    {
        
        $tech= Service::find($id)->delete();
    ...
    public function getServices(Request $request)
    {
        ...
        $searchValue = $search_arr['value']; // Search value
            ...
            // Total records
            $totalRecord = Service::where('deleted_at', null);
            ...
            $totalRecordswithFilte = Service::where('deleted_at', null);
            ...
            $items = Service::where('deleted_at', null)->orderBy($columnName, $columnSortOrder);
            if ($title != "") {
        ...
        foreach ($records as $record) {
            ...
            $data_arr[] = [
                ...
                "image" => '<a href="/Services/'.$image.'" target="_blank">'.$image.'</a>',
                "edit" => '<div class="settings-main-icon">
                              <a class="deleteItem" data-id="' . $id . '"><i class="fa fa-trash bg-danger "></i></a>
                              <a href="' . route('service_list.edit', $id) . '"><i class="fa fa-edit bg-info me-1"></i></a>
                           </div>'
            ];
